...
export async function POST(req: Request) {
  try {
    ...
    const batchSize = batchDoc && batchDoc.batchsize ? parseInt(batchDoc.batchsize, 10) : 10;
    const batchDelay = batchDoc && batchDoc.batchdelay ? parseInt(batchDoc.batchdelay, 10) : 1000;
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // send each recipient in the batch
      for (const r of batch) {
        try {
          ...
          results.push({ email: r.email, status: 'error', message: err.message });
        }
      }
    ...
    return NextResponse.json({ code: 101, message: err.message }, { status: 500 });
  }
}
